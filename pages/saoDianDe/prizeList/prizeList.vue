<template>
	...
	<view class="winningRecord flex-xn-ys">
		<uni-nav-bar :style="{ 'margin-top': safeAreaTop*2 + 'rpx' }" left-icon="back" @click-left="back" title="我的实物奖"></uni-nav-bar>
			...
			<scroll-view v-if="isHasData" :style="{ height: scrollViewHeight + 'px' }" scroll-y="true" class="wrc-listBox">
				...
				<view class="wrc-listBox-list flex-xsb-yn" v-for="(item,index) in prizeRecordAry" :key="index">
					<view class="flex-xn-yc">
						...
						<view class="">
							<view class="wrc-listBox-list-mes">{{ item.prizeName }}</view>
		...
		<view class="wr-title-box flex-xc-yc" v-if="tabIsCur == 0">
			<view class="footer-mes">
				温馨提示：所有中出奖品均有领取有效期，超出有效期未领取则 视为放弃该奖品，请尽快领取,活动最终解释权归青岛啤酒所有。
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			initData() {
				// 先这样 后期 考虑缓存数据
				const that = this;
				queryPrizeList(that.sendData.currentPage, that.sendData.count, that.sendData.useStatus).then(res => {
					console.log('queryExchangePrizeAllLst');
					console.log(res);
					if (res.prizeRecordAry.length < 1) {
						that.isHasData = false;
					} else {
						that.isHasData = true;
					}
					console.log(res.prizeRecordAry.length);
					console.log(that.isHasData);
					// 判断 是否有下一页 拿 that.count 和当前数组长度 比较 
					if (res && (!res.prizeRecordAry || res.prizeRecordAry.length < that.sendData.count)) {
						that.moneyNext = false;
					} else {
						that.moneyNext = true;
					}
					if (that.sendData.currentPage == 1) {
						that.prizeRecordAry = res.prizeRecordAry;
					} else {
