<template>
	...
	<view class="blacklist flex-xn-ys">
		...
		<view class="bl-center" v-if="showType">
			<view class="">
				...
				<!-- <view class="flex-xc-yn">
					<image mode="widthFix" class="blc-crownCookies" :src="commoniImg+'erweima.png'"></image>
				</view> -->
			</view>
		</view>
		<view class="bl-center" v-else>
			<view class="">
				<view class="flex-xc-yn">
					<image mode="widthFix" class="blc-logo" :src="staticUrl + 'crownCookiesImgLogo.png'"></image>
				</view>
				<p class="blc-title">您的账号存在可疑风险，为确保您的账号安全，</p>
				<p class="blc-p">
					<text class="blc-text">1、关注【青岛啤酒畅享山东公众号】，</text>
					获得更多活动信息，请截图保存该图片，并在手机相册中进行识别二维码。
				</p>
				<p class="blc-p" @click='callGetPhoneFun'>2、客服电话：{{ callGetPhone }}</p>
				<view class="flex-xc-yn">
					<image mode="widthFix" :style="{ marginottom: isOpenAdaptation ? '210rpx' : '54rpx' }" class="blc-officialAccount"
					 :src="staticUrl+'erweima.jpeg'"></image>
				</view>
				<view class="flex-xc-yn">
					<image mode="widthFix" class="blc-crownCookies" :src="commoniImg + 'erweima.png'"></image>
				</view>
			...
			submitForm() {
				if (!reg1.test(this.phonenum)) {
					...
					updateUserInfoMobile(openid, this.phonenum, this.captcha)
						.then(res => {
							if (res.result.businessCode == 0) {
								const sweepstr =  getApp().globalData.sweepstr ;
...
<style lang="scss">
	...
	.blacklist {
		height: 100%;
		background: url($commonImg+'abnormalBg.png') no-repeat center;
